...
/* ------------------------------------------------------------------ */
/* RevealLayer: cursor-following spotlight that reveals a second image */
/* ------------------------------------------------------------------ */
function RevealLayer({
  ...
  // On every render: paint the soft radial gradient mask, then apply it
  useEffect(() => {
    const canvas = canvasRef.current
    if (!canvas) return
  ...
  // Keep the reveal hidden until a real mask exists (avoids an initial flash)
  const mask = maskUrl || 'linear-gradient(rgba(0,0,0,0), rgba(0,0,0,0))'
